...
public function admindashboard()
{
    ...
    //bot history
    $totalBotsPurchased = BotHistory::count();
    // Users
  ...
  public function updateWallet()
    {
        ...
        return view('admin.payment.update_wallet');
    }
    ...
    public function chooseWallet(Request $request)
    {
        ...
        if ($method == 'btc') {
            ...
            return view('admin.payment.litecoin');

        } elseif ($method == 'solana') {

            return view('admin.payment.solana');

        } elseif ($method == 'bnb') {

            return view('admin.payment.bnb');
        } elseif ($method == 'xrp') {

            return view('admin.payment.xrp');
        } else {
  ...
  public function updateTrc(Request $request)
{
    ...
    // Update USDT address
    $admin->usdt_address = $request->input('usdt_address');

    // Handle USDT image upload
    if ($request->hasFile('image')) {
        $file = $request->file('image');
        $ext = $file->getClientOriginalExtension();
        $filename = time() . '.' . $ext;
        $file->move('admin/uploads/admin', $filename);
        $admin->usdt_image = $filename; // ✅ Correct field name
    }
    ...
    $userData = [
        'usdt_address' => $admin->usdt_address,
        'usdt_image' => $admin->usdt_image,
    ];

    return view('admin.payment.usdt', compact('userData'))
        ->with('status', 'USDT Details Updated Successfully');
}

   public function updateBtc(Request $request)
{
    // Use the 'admin' guard to get the logged-in admin
    $admin = Auth::guard('admin')->user();

    // Update the BTC address
    $admin->btc_address = $request->input('btc_address');
    ...
    if ($request->hasFile('image')) {
        ...
        $ext = $file->getClientOriginalExtension();
        $filename = time() . '.' . $ext;
        $file->move(public_path('admin/uploads/admin'), $filename);
        $admin->btc_image = $filename;
    }
    ...
    $userData = ['btc_address' => null, 'btc_image' => null];

    // If any changes were made, pass the updated values
    if ($admin->wasChanged()) {
        $userData['btc_address'] = $admin->btc_address;
        $userData['btc_image'] = $admin->btc_image;
    }

    // Return the view with data and a success message
    return view('admin.payment.btc', compact('userData'))->with('status', 'Bitcoin Details Updated Successfully');
}
   ...
   public function updateEth(Request $request)
{
    ...
    // Update ETH address
    $admin->eth_address = $request->input('eth_address');

    // Handle ETH QR/Image upload
    if ($request->hasFile('image')) {
        $file = $request->file('image');
        $ext = $file->getClientOriginalExtension();
        $filename = time() . '.' . $ext;
        $file->move('admin/uploads/admin', $filename);
        $admin->eth_image = $filename; // ✅ Match column name exactly
    }
    ...
    $userData = [
        ...
        'eth_image' => $admin->eth_image,
    ];

    return view('admin.payment.eth', compact('userData'))
        ->with('status', 'Ethereum Details Updated Successfully');
}
   ...
   public function updateLitecoin(Request $request)
{
    ...
    // Update Litecoin address
    $admin->litecoin_address = $request->input('litecoin_address');

    // Handle Litecoin QR/Image upload
    if ($request->hasFile('image')) {
        $file = $request->file('image');
        $ext = $file->getClientOriginalExtension();
        $filename = time() . '.' . $ext;
        $file->move('admin/uploads/admin', $filename);
        $admin->litecoin_image = $filename; // ✅ Correct column name
    }
    ...
    // Prepare user data for view
    $userData = [
        'litecoin_address' => $admin->litecoin_address,
        'litecoin_image' => $admin->litecoin_image,
    ];

    return view('admin.payment.litecoin', compact('userData'))
        ->with('status', 'Litecoin Details Updated Successfully');
}
   ...
   public function updateSolana(Request $request)
{
    ...
    $admin->solana_address = $request->input('solana_address');

    // Handle Solana QR/Image upload
    if ($request->hasFile('image')) {
        $file = $request->file('image');
        $ext = $file->getClientOriginalExtension();
        $filename = time() . '.' . $ext;
        $file->move('admin/uploads/admin', $filename);
        $admin->solana_image = $filename; // ✅ Correct field name
    }
    ...
    // Prepare user data for view
    $userData = [
        'solana_address' => $admin->solana_address,
        'solana_image' => $admin->solana_image,
    ];

    return view('admin.payment.solana', compact('userData'))
        ->with('status', 'Solana Details Updated Successfully');
}
...
public function updateBnb(Request $request)
{
    ...
    // Update BNB address
    $admin->bnb_address = $request->input('bnb_address');

    // Handle BNB image upload
    if ($request->hasFile('image')) {
        $file = $request->file('image');
        $ext = $file->getClientOriginalExtension();
        $filename = time() . '.' . $ext;
        $file->move('admin/uploads/admin', $filename);
        $admin->bnb_image = $filename; // ✅ Correct column name
    }
    ...
    // Prepare user data for view
    $userData = [
        'bnb_address' => $admin->bnb_address,
        'bnb_image' => $admin->bnb_image,
    ];

    return view('admin.payment.bnb', compact('userData'))
        ->with('status', 'BNB Details Updated Successfully');
}
   ...
   public function updateXrp(Request $request)
{
    ...
    // Update XRP address
    $admin->xrp_address = $request->input('xrp_address');

    // Handle XRP image upload
    if ($request->hasFile('image')) {
        $file = $request->file('image');
        $ext = $file->getClientOriginalExtension();
        $filename = time() . '.' . $ext;
        $file->move('admin/uploads/admin', $filename);
        $admin->xrp_image = $filename; // ✅ Correct field name
    }
    ...
    // Prepare user data for view
    $userData = [
        'xrp_address' => $admin->xrp_address,
        'xrp_image' => $admin->xrp_image,
    ];

    return view('admin.payment.xrp', compact('userData'))
        ->with('status', 'XRP Details Updated Successfully');
}
